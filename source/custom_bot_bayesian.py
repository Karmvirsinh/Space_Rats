...
def custom_bayesian_bot(ship, alpha=0.15):
    ...
    senses += 1
    print(f"Step 1: Sensed surroundings, Senses: {senses}")  # Log senses
    ...
    while len(bot_knowledge) > 1 and step < max_steps:
        ...
        moves += 1
        print(f"Step {step}: Moved {chosen_move} to {new_pos}, Moves: {moves}")  # Log moves
        bot_pos = new_pos
        current_sensor = get_surroundings(ship, bot_pos)
        senses += 1
        print(f"Step {step}: Sensed surroundings, Senses: {senses}")  # Log senses
        blocked_count = sum(1 for x in current_sensor if x == 0)
    ...
    while bot_pos != true_rat_pos and step < max_steps:
        if not target_path:
            if not rat_knowledge:
                ...
                print(f"Final Metrics - Moves: {moves}, Senses: {senses}, Pings: {pings}")  # Log final metrics
        ...
        moves += 1
        if new_pos != bot_pos:
            print(f"Step {step}: Moved {direction} to {new_pos}, Moves: {moves}, Rat KB size: {len(rat_knowledge)}")  # Log moves
        bot_pos = new_pos

        # Check catch before rat moves
        if bot_pos == true_rat_pos:
            pings += 1
            print(f"Step {step}: Pinged at {bot_pos}, Pings: {pings}")  # Log pings
