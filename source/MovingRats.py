...
def simple_baseline_bot(ship, alpha, moving_rat=False):
    ...
    step = 2  # Start at step 2 to match previous scripts
...
def evaluate_bots(ship_size=30, num_trials=50):
    alpha_values = np.arange(0.0, 0.90, 0.05)  # 0.05, 0.1, ..., 0.7 (to match previous scripts)
    baseline_avg_moves = []
    custom_avg_moves = []
    ...
    for alpha in alpha_values:
        ...
        print(f"Alpha = {alpha:.2f} done, Avg Moves (Simple Baseline) = {baseline_avg:.2f}, Avg Moves (Custom Bayesian) = {custom_avg:.2f}")
...
def plot_comparison(alpha_values, baseline_avg_moves, custom_avg_moves, ship_size, num_trials):
    ...
    plt.plot(alpha_values, baseline_avg_moves, marker='o', label='Simple Baseline (Moving Rat)', color='skyblue')
    plt.plot(alpha_values, custom_avg_moves, marker='o', label='Custom Bayesian (Moving Rat)', color='salmon')
    plt.xlabel("Alpha")
